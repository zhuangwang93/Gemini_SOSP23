...
def train(args,
    ...
    for _, batch_index in enumerate(tqdm(dataset_iterator, smoothing=1)):
        ...
        logger.info(f"[Training] step time: {step_time}")
        ...
        if dist.get_rank() == 0:
            ...
            if snapshot_settings.is_comm_profile_mode():
...
def prepare_model_optimizer(args):
    ...
    logger.info(f"[total number of parameters] {count_parameters(model.network)}")
    time.sleep(5)
...
def run(args, model, optimizer, start_epoch):
    ...
    training_profiler.set_training_init_time()
    if dist.get_rank() == 0:
        training_profiler.report()
            
    for index in range(start_epoch, config["training"]["num_epochs"]):
        logger.info(f"Training Epoch: {index + 1}")
        pre = time.time()

        snapshot_settings.set_global_epoch(index + 1)
