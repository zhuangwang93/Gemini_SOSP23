...
class DeepSpeedEngine(Module):
    ...
    def __init__(
        ...
        # Zhuang: record checkpoint filename for recovery
        self.checkpoint_path = args.saved_model_path
        self.snapshot_path = args.snapshot_path
        self.set_zero_checkpoint_filename()
    

    def set_zero_checkpoint_filename(self):
        os.makedirs(self.checkpoint_path, exist_ok=True)
        from deepspeed import comm as dist
        local_rank = dist.get_local_rank()
        if local_rank == 0:
            checkpoint_filename = os.path.join(self.checkpoint_path, "checkpoint_filename")
            name = self.get_zero_checkpoint_filename()
            import json
            with open(checkpoint_filename, "w") as f:
                json.dump(name, f)
        snapshot_path = os.path.join(self.checkpoint_path, self.snapshot_path)
        os.makedirs(snapshot_path, exist_ok=True)


    def get_zero_checkpoint_filename(self):
        filename = self._get_zero_ckpt_name("", "")
