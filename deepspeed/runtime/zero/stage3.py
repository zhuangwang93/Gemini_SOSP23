...
from deepspeed.runtime.snapshot.snapshot_global_variables import snapshot_settings, comm_profiler
...
class DeepSpeedZeroOptimizer_Stage3(ZeROOptimizer):
    ...
    def __avg_scatter_grads(self, params_to_reduce: List[Parameter]) -> List[Tensor]:
        ...
        start.record(stream=self.__reduce_and_partition_stream)
        handle, grad_partitions_for_rank = reduce_scatter_coalesced(full_grads_for_rank,
    ...
    def snapshot_states(self):
        from deepspeed.runtime.snapshot.snapshot_comm import cpu_snapshot
        cpu_snapshot.update_local_optimizer_snapshot(self.state_dict())
